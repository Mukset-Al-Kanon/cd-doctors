      ...
      >
        ...
        <div className="bg-white/80 backdrop-blur-xl text-slate-900 p-3.5 sm:p-4 flex items-center justify-between shrink-0 border-b border-slate-200/60 w-full min-w-0 box-border">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <div className="relative w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-gradient-to-tr from-sky-500 to-sky-400 flex items-center justify-center shadow-sm text-white shrink-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 truncate">
                <h3 className="font-extrabold text-base sm:text-lg text-slate-900 tracking-tight truncate">CD Doctors</h3>
                <span className="bg-sky-100/80 text-sky-800 text-[10px] sm:text-[11px] font-extrabold px-1.5 py-0.5 rounded-md border border-sky-200/80 shrink-0">
                  Assistant
                </span>
              </div>
              <p className="text-xs sm:text-xs text-slate-500 font-medium truncate">চুয়াডাঙ্গা স্বাস্থ্যসেবা তথ্য সহায়ক</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100/80 rounded-xl transition-colors active:scale-95"
            >
          ...
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex items-start gap-2.5 min-w-0 ${
                    hasRichCards
                      ? 'w-full max-w-full'
                      : msg.sender === 'user'
                      ? 'max-w-[88%] sm:max-w-[80%] flex-row-reverse'
                      : 'max-w-[95%] sm:max-w-[88%]'
                  }`}
                >
                  {/* Sender Avatar */}
                  <div
                    className={`w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center shrink-0 shadow-xs mt-0.5 ${
                      msg.sender === 'user'
                  ...
                  {/* Message Bubble with Readable Mobile Text */}
                  <div
                    className={`p-3.5 sm:p-4 rounded-2xl text-[14px] sm:text-[14.5px] font-medium leading-[1.65] shadow-xs flex flex-col gap-2 min-w-0 w-full box-border break-words overflow-hidden ${
                      msg.sender === 'user'
                      ...
                      <div className="space-y-2.5 w-full min-w-0 pt-1">
                        {msg.hospitals.map((hosp, idx) => (
                          <div
                            ...
                            className="bg-white border border-sky-100 hover:border-sky-300 rounded-2xl p-3.5 sm:p-4 shadow-sm transition-all duration-200 flex flex-col gap-2.5 min-w-0 w-full box-border overflow-hidden animate-card-pop"
                          >
                            <div className="flex items-start justify-between gap-2 min-w-0">
                              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <span className="text-[10px] sm:text-[11px] font-bold text-sky-800 bg-sky-100/80 px-2 py-0.5 rounded-md border border-sky-200 inline-block mt-0.5 truncate max-w-full">
                                    {hosp.hospitalType}
                                  </span>
                                </div>
                              </div>
                            ...
                            <div className="flex items-start gap-1.5 text-xs sm:text-sm text-slate-600 font-medium break-words min-w-0">
                            ...
                            <div className="flex flex-wrap gap-1 pt-0.5">
                              {hosp.hasEmergency && (
                                <span className="text-[11px] font-bold bg-rose-50 text-rose-700 border border-rose-200 px-2.5 py-0.5 rounded-md flex items-center gap-1">
                                  <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" />
                                  24/7 Emergency
                                </span>
                              )}
                              {hosp.hasIcu && (
                                <span className="text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-0.5 rounded-md">
                                  ICU Facility
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 pt-1 border-t border-slate-100 min-w-0">
                              <Link
                                ...
                                onClick={() => setIsOpen(false)}
                                className="flex-1 min-w-0 flex items-center justify-center gap-1 bg-slate-900 hover:bg-sky-700 text-white text-xs sm:text-sm font-bold py-2.5 px-3 rounded-xl transition-colors shadow-2xs truncate"
                              >
                              ...
                              {hosp.phone && (
                                <a
                                  href={`tel:${hosp.phone}`}
                                  className="flex-1 sm:flex-none min-w-0 flex items-center justify-center gap-1 bg-sky-600 hover:bg-sky-700 text-white text-xs sm:text-sm font-bold py-2.5 px-3 rounded-xl transition-colors shadow-2xs truncate"
                                >
                      ...
                      <div className="space-y-3 w-full min-w-0 pt-1">
                        {msg.doctors.map((doctor, idx) => (
                          <div
                            key={doctor.id}
                            style={{ animationDelay: `${idx * 100}ms` }}
                            className="bg-white border border-slate-200 hover:border-sky-300 rounded-2xl p-3.5 sm:p-4 shadow-sm transition-all duration-200 flex flex-col gap-2.5 min-w-0 w-full box-border overflow-hidden animate-card-pop"
                          >
                            <div className="flex items-start gap-3 min-w-0">
                              ...
                              <div className="flex-1 min-w-0 space-y-0.5">
                                <span className="bg-sky-50 text-sky-700 font-extrabold text-[11px] sm:text-xs px-2 py-0.5 rounded-lg border border-sky-100/90 inline-block mb-0.5 truncate max-w-full">
                                ...
                                <p className="text-xs sm:text-xs text-slate-500 font-medium break-words">
                                  {doctor.degrees}
                                </p>
                                <p className="text-xs sm:text-xs font-bold text-sky-700 break-words">
                                  {doctor.specialization}
                                </p>
                              </div>
                            </div>

                            <div className="bg-slate-50/80 border border-slate-100 rounded-xl p-2.5 space-y-1.5 text-xs sm:text-sm min-w-0">
                              <div className="flex items-center gap-1.5 font-bold text-slate-800 text-xs sm:text-sm truncate min-w-0">
                              ...
                              {doctor.schedules && doctor.schedules.length > 0 && (
                                <div className="pt-0.5">
                                  <span className="text-[11px] sm:text-xs font-bold text-slate-600 block mb-1">
                                    📅 চেম্বারের দিনসমূহ:
                                  </span>
                                  <div className="flex flex-wrap gap-1">
                                    {[
                                      ...
                                      return (
                                        <span
                                          key={d.num}
                                          className={`text-[10px] sm:text-[11px] font-extrabold px-2 py-0.5 rounded-md ${
                                            isAvailable
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1 text-xs sm:text-xs pt-1 text-slate-600 font-bold border-t border-slate-200/60 mt-1 min-w-0">
                      ...
                      <div className="space-y-2.5 w-full min-w-0 pt-1">
                        {msg.bloodDonors.map((donor, idx) => (
                          <div
                            key={donor.id}
                            style={{ animationDelay: `${idx * 100}ms` }}
                            className="bg-white border border-rose-100 hover:border-rose-300 rounded-2xl p-3.5 sm:p-4 shadow-sm transition-all duration-200 flex flex-col gap-2 min-w-0 w-full box-border overflow-hidden animate-card-pop"
                          >
                            <div className="flex items-center justify-between gap-2 min-w-0">
                              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-rose-500 text-white font-black text-sm sm:text-base flex items-center justify-center shadow-xs shrink-0">
                                  {donor.bloodGroup}
                                </div>
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className="text-[11px] sm:text-xs text-emerald-600 font-bold flex items-center gap-1 truncate">
                            ...
                            <div className="flex items-start gap-1.5 text-xs sm:text-sm text-slate-600 font-medium bg-slate-50 p-2.5 rounded-xl border border-slate-100 break-words min-w-0">
                      ...
                      <div className="space-y-2.5 w-full min-w-0 pt-1">
                        {msg.emergencyServices.map((em, idx) => (
                          <div
                            key={em.id}
                            style={{ animationDelay: `${idx * 100}ms` }}
                            className="bg-gradient-to-br from-white to-amber-50/40 border border-amber-200/80 rounded-2xl p-3.5 sm:p-4 shadow-sm transition-all duration-200 flex flex-col gap-2 min-w-0 w-full box-border overflow-hidden animate-card-pop"
                          >
                            <div className="flex items-start justify-between gap-2 min-w-0">
                              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <span className="text-[10px] sm:text-[11px] font-bold text-amber-800 bg-amber-100/80 px-2 py-0.5 rounded-md border border-amber-200 inline-block mt-0.5 truncate max-w-full">
                                    {em.badge}
                                  </span>
                                </div>
                              </div>
                            ...
                            <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed break-words min-w-0">{em.desc}</p>

                            <a
                              href={`tel:${em.number}`}
                              className="w-full flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-700 text-white text-xs sm:text-sm font-bold py-2.5 px-3 rounded-xl transition-colors shadow-2xs truncate"
                            >
                              <PhoneCall className="w-3.5 h-3.5 shrink-0" />
                ...
                {/* Timestamp */}
                <span className="text-[11px] sm:text-xs text-slate-400 mt-1 px-9 font-medium">{msg.timestamp}</span>
              </div>
            );
          })}
        ...
        <div className="p-3 sm:p-3.5 bg-white/80 backdrop-blur-xl border-t border-slate-200/60 shrink-0 w-full min-w-0 box-border">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleSendMessage();
          ...
          >
            <input
              ...
              className="flex-1 min-w-0 bg-white/95 border border-slate-200/90 focus:border-sky-500 focus:bg-white text-slate-800 text-sm sm:text-sm md:text-base rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-sky-400/20 transition-all font-medium placeholder:text-slate-400 shadow-2xs"
            />
          ...
          <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500 font-medium px-1 mt-1.5">
